...
$template->assign_var('COL_P', $log->get("premium"));
...
if($gameCore['twitchon'])
	$template->assign_var('TWITCHON', '');
else
	$template->assign_var('TWITCHON', 'GTFO');
...
if($pj)
{
	if($pj['SubClassFrom']>0)
	{
		...
		$logros = $db->sql_fetchrow($logosq);
			$template->assign_var('SUBCLASS_OPTION', "");
	}
	else
	{
		$query = 'SELECT *
					FROM logros WHERE idPersonaje = '.$log->get("pjSelected").'';
		$template->assign_var('SUBCLASS_OPTION', "GTFO");
	}
	...
	$template->assign_var('NPCAURALVL', $logros['NPCaura']);
	...
	//// PARTY ////////////////
		$template->assign_var('PARTYOWENER', 0);
	if($pj['party']>0)
	{
		if($pj['party']==$log->get("pjSelected"))
		{
			$template->assign_var('PARTYOWENER', 1);
...
if($gameCore['war'] && $pj['clan']>0)
{
	...
	if($pj['nivel']>=90)
		...
		if(isset($_COOKIE['PARTY_TOP']))
			$template->assign_var('VOID_TOP', $_COOKIE['VOID_TOP']);
		else
			$template->assign_var('VOID_TOP', 748);
		
		if(isset($_COOKIE['VOID_LEFT']))
			$template->assign_var('VOID_LEFT', $_COOKIE['VOID_LEFT']);
		else
			$template->assign_var('VOID_LEFT', 193);
...
<script id="sid0020000048180303027">(function() {function async_load(){s.id="cid0020000048180303027";s.src='."'".'http://st.chatango.com/js/gz/emb.js'."'".';s.style.cssText="width:263px;height:479px;";s.async=true;s.text='."'".'{"handle":"swordon","arch":"js","styles":{"a":"383838","b":100,"c":"FFFFFF","d":"FFFFFF","k":"383838","l":"383838","m":"383838","n":"FFFFFF","q":"383838","r":100,"usricon":0.92,"cnrs":"1"}}'."'".';var ss = document.getElementsByTagName('."'".'script'."'".');for (var i=0, l=ss.length; i < l; i++){if (ss[i].id=='."'".'sid0020000048180303027'."'".'){ss[i].id +='."'".'_'."'".';ss[i].parentNode.insertBefore(s, ss[i]);break;}}}var s=document.createElement('."'".'script'."'".');if (s.async==undefined){if (window.addEventListener) {addEventListener('."'".'load'."'".',async_load,false);}else if (window.attachEvent) {attachEvent('."'".'onload'."'".',async_load);}}else {async_load();}})();</script>');
	...
	$template->assign_var('CANUSE2HAND', $stats['Allow2Hand']);

	$pj['imagen'] = $pj['imagen'].'_'.$pj['sexo'].'.png';
	...
	if($pj['SubClassFrom']>0)
	{
		$template->assign_var('ID_PBASE', $pj['SubClassFrom']);
		$idPersonajeBase = $pj['SubClassFrom'];
	}
	else
	{
		$template->assign_var('ID_PBASE', $pj['idPersonaje']);
	...
	////SILENCE
	 ...
	 $template->assign_var('CHAT_SILENCE', $chatBnList);
	 $template->assign_vars(array(
			  ...
			  'NOMBRE_P' => $pj['nombre'],
			   'ILVL' => $stats['ItemLevel'],
				...
				//CLAN CHECK
				$clan=false;
				$template->assign_var('CLANLIDER', 0);
				if($pj['clan']>0)
				{
					$query = 'SELECT *
					...
					$template->assign_var('CLANID', $pj['clan']);

					if($clan['idLeader']==$log->get("pjSelected") || $clan['inviteRight']==1)
					{
							$template->assign_var('CLANLIDER', 1);
					}
					...
					if($pj['Vida']<=0 OR $pj['deathTime']>$now)
					{
						...
						if($pj['deathTime']<=$now AND isset($_GET['revivir']))
						{
							if(isset($_GET['nomerompanloshuevos']) AND $pj['pvpKilledTime']>$now)
							{
								...
								$msg = "<div class='recompensaAstral'>".$pj['nombre']." apret&oacute; el bot&oacute;n del cobarde! <img src='images/cobarde.png' /></div>";	
						...
						switch($_GET['sec'])
						{
							...
							break;
							case 'godlevel':
								include('includes/section/godLevel.php');
							break;
							case 'news':
...
$_SESSION['soulImpactUsed']=0;
					...
					else
					{
							...
							break;
							case 'thevoid':
								include('includes/section/thevoid.php');
							break;
							case 'subclass':
								include('includes/section/subclass.php');
							break;
							...
							break;
							case 'subclassSkills':
								include('includes/section/subclass_skills.php');
							break;
							case 'clanShop':
								include('includes/section/clanShop.php');
							break;
						...
						break;							
							...
							break;
							case 'silence':
								include('includes/section/silence.php');
							break;	
							break;
							case 'tienda':
								$template->set_filenames(array(
					...
					show_error("El nombre del clan (".$clan.") tiene que tener entre 3 y 9 caracteres","index.php?sec=crearclan");
							...
							case 'ResetSkills':
								if($pj['attackCooldown']<$now-120)
								{
									...
									if($pj['SubClassFrom']>0)
									{
										$query = 'SELECT paragonAcc
										FROM personaje 
										WHERE '.$pj['SubClassFrom'].' = idPersonaje';
											$mainthingsq = $db->sql_query($query);
										$mainthing = $db->sql_fetchrow($mainthingsq);
										$parPerLv = $mainthing['paragonAcc'];
									}
