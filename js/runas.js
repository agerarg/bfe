...
runaUsada= (i)=>{
	if(checkRuneType(itemsCatch[i]['idItem']))
	{
		jAlert("Ya tienes equipada una runa de este tipo!", 'Error');	
	}else
	if(itemsCatch[i]['enVenta']==1)
	{
		jAlert("Esta runa esta en venta!", 'Error');	
	}else
	if(runaIndex<=limiteRunasCt) 
	{
		...
		runaIndex++;
		if(itemsCatch[i]['usadoPor']==0)
		{
			//alert("id="+itemsCatch[i]['idInventario']+"&ac=poner");
			$.ajax({
				data: "id="+itemsCatch[i]['idInventario']+"&ac=poner",
				type: "GET",
				dataType: "json",
				url: "json/setRuna.php"
			});
		}
...
sacarRuna=(rIndex,ItemI,openRuna)=>{
	...
	runaIndex--;
	$.ajax({
		data: "id="+itemsCatch[ItemI]['idInventario']+"&ac=sacar",
		type: "GET",
		dataType: "json",
		url: "json/setRuna.php"
	});
