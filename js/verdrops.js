		...
		while(itemsCatch[i])
			{
				...
				if(cantidad>1)
				cant_show=cantidad+" ";	 
				$("#item_list").append("<div class='itemObj itemObjOPEN' id='objContent"+i+"'><div class='item_img "+setInfo+"' title='"+title+"' id='"+i+"'><img src='images/item/"+itemsCatch[i]['subtipo']+"/"+itemsCatch[i]['imagen']+"' /></div><div class='item_nombre'>"+cant_show+itemsCatch[i]['Nombre']+enchant+"</div><div class='item_mains'><div class='item_desc'>"+makeDesc(itemsCatch[i]," ")+"</div></div></div>");
...
function checkItems()
	{
		$.ajax({
			...
			type: "GET",
			dataType: "json",
			url: "json/verdrops.php?id="+monsterDrop,
