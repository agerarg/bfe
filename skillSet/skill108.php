...
switch($skill['nivel'])
{
	case 1:
		if($stats['DestroActive']==1)
		{
			...
			$ataque_player = $stats['Ataque']*2 + $stats['SkillDmg'];
		if($stats['InfinityEadge']==1)
		{
			$ataque_player +=$stats['IEAcumulate']*($ataque_player/50);
			
			
				insertBuff($pj['idPersonaje'],193,114,120);
				$data['aura'][] = array("idSkill"=>114,"lvl"=>1,"auraTimeOut"=>120,"pasive"=>0);
...
if($stats['SwordBlastDmg']>0)
	$ataque_player=potenciar($ataque_player,$stats['SwordBlastDmg']);
